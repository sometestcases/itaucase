...
- Apesar de solicitado, não contemplei mecanismo de 'rate limit',  porque esses micro-serviços, estariam em rede interna, não seriam expostos, e cada canal/produto que for utilizar vai ter seu 'BFF' que será exposto, e nesse caso o rate limit já vai estar nesses serviços que são expostos que terão o IP do cliente. Sei que mesmo assim o rate limit ainda seria aplicavel nos micro-serviços, porém o IP que esses micro-serviços desse projeto teriam não seria do cliente, e sim das instancias de outras aplicações, ou seja, não seria dos melhores indicadores para ser a chave do 'rate limit', porque quando um IP for limitado ira estar impactando varias requisições que não tem necessáriamente relação com o exceço de chamadas, nesse caso seria melhor fazer isso através de um Header de Origem por exemplo mas que cai no mesmo problema, então para gerar um beneficio real e não piorar a situação ao invés de melhorar, é algo que precisaria ser avaliado com mais calma, como a principio não impacta na demostração do case e eu tive que selecionar bem o que iria ser feito (por conta do prazo e de a arquitetura ter ficado grande pra implementar), eu descartei isso (porque como comentei é mais um mecanismo de garantia mas na pratica já deve ser resolvido antes de chegar nesses serviços), e se tiverem muitas requisições a aplicação escala .
